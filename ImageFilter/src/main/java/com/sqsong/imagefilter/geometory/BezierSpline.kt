package com.sqsong.imagefilter.geometory
...
fun curveGenerator(knots: Array<PointF>?): IntArray {
    if (knots == null) {
        throw NullPointerException("Knots cannot be null")
    }
    val n = knots.size - 1
    require(n >= 1) { "Atleast two points are required" }
    return getOutputPointsForNewerDevices(knots)
}


private fun getOutputPointsForNewerDevices(knots: Array<PointF>): IntArray {
    val controlPoints: Array<PointF> = calculateControlPoints(knots)
    val path = Path()
    path.moveTo(0f, 0f)
...
private fun validateCurve(allPoints: FloatArray): IntArray {
    ...
    for (x in 0..255) {
        if (allPoints[x] > 255.0f) {
            ...
            curvedPath[x] = allPoints[x].roundToInt()
        }
    }
    return curvedPath
}

private fun calculateControlPoints(knots: Array<PointF>): Array<PointF> {
    val n = knots.size - 1
    val controlPoints: Array<PointF> = Array(n) { PointF() }
    if (n == 1) { // Special case: Bezier curve should be a straight line.
        // 3P1 = 2P0 + P3
        controlPoints[0] = PointF((2 * knots[0].x + knots[1].x) / 3, (2 * knots[0].y + knots[1].y) / 3)
        // P2 = 2P1 – P0
        ...
        for (i in 0 until n) {
            controlPoints[i] = PointF(x[i], y[i])
        }
    }
