...
public final class EglBuffer {
    ...
    public Bitmap getBlurBitmap(Bitmap bitmap, int radius, @Mode int mode) {
        ...
        OffScreenBlurRenderer renderer;
        try {
            ...
            renderer.onDrawFrame(bitmap, true);
    ...
    public Bitmap getMosaicBitmap(Bitmap bitmap, float mosaicSize) {
        final int w = bitmap.getWidth();
        final int h = bitmap.getHeight();
        EGLDisplay eglDisplay = EGL10.EGL_NO_DISPLAY;
        EGLSurface eglSurface = null;
        EGLContext eglContext = null;
        EGLConfig[] eglConfigs = new EGLConfig[1];
        MosaicRender renderer;
        try {
            eglDisplay = createDisplay(eglConfigs);
            eglSurface = createSurface(w, h, eglDisplay, eglConfigs);
            eglContext = createEGLContext(eglDisplay, eglConfigs);
            EGL.eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext);
            renderer = createRectMosaicRenderer(mosaicSize);
            renderer.onDrawFrame(bitmap, false);
            EGL.eglSwapBuffers(eglDisplay, eglSurface);
            convertToBitmap(bitmap);
        } catch (Throwable t) {
            Log.e(TAG, "Mosaic the bitmap error", t);
        } finally {
            destroyEglSurface(eglDisplay, eglSurface);
            destroyEglContext(eglDisplay, eglContext);
        }
    ...
    public Bitmap getLutFilterBitmap(Bitmap bitmap, Bitmap lutBitmap, float intensity) {
        final int w = bitmap.getWidth();
        final int h = bitmap.getHeight();
        EGLDisplay eglDisplay = EGL10.EGL_NO_DISPLAY;
        EGLSurface eglSurface = null;
        EGLContext eglContext = null;
        EGLConfig[] eglConfigs = new EGLConfig[1];
        LutFilterRender renderer;
        try {
            eglDisplay = createDisplay(eglConfigs);
            eglSurface = createSurface(w, h, eglDisplay, eglConfigs);
            eglContext = createEGLContext(eglDisplay, eglConfigs);
            EGL.eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext);
            renderer = new LutFilterRender();
            renderer.setLutTextureBitmap(lutBitmap, intensity);
            renderer.onDrawFrame(bitmap, false);
            EGL.eglSwapBuffers(eglDisplay, eglSurface);
            convertToBitmap(bitmap);
        } catch (Throwable t) {
    ...
    public Bitmap getTestBitmap(Bitmap bitmap) {
        final int w = bitmap.getWidth();
        final int h = bitmap.getHeight();
        EGLDisplay eglDisplay = EGL10.EGL_NO_DISPLAY;
        EGLSurface eglSurface = null;
        EGLContext eglContext = null;
        EGLConfig[] eglConfigs = new EGLConfig[1];
        TestShaderRender renderer;
        try {
            eglDisplay = createDisplay(eglConfigs);
            eglSurface = createSurface(w, h, eglDisplay, eglConfigs);
            eglContext = createEGLContext(eglDisplay, eglConfigs);
            EGL.eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext);
            renderer = new TestShaderRender();
            renderer.onDrawFrame(bitmap, false);
            EGL.eglSwapBuffers(eglDisplay, eglSurface);
            convertToBitmap(bitmap);
        } catch (Throwable t) {
    ...
    private MosaicRender createRectMosaicRenderer(float mosaicSize) {
        MosaicRender renderer = new MosaicRender();
        renderer.setMosaicSize(mosaicSize);
        return renderer;
    }
}
