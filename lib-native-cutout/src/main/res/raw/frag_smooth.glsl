...
void main() {
    ...
    centralColor = texture2D(inputImageTexture, textureCoordinate).rgba;
    ...
    if (opacity < 0.01) {
        ...
        vec3 smoothColor = centralColor.rgb * aa - vec3(sampler) * (aa - 1.0);
        ...
        smoothColor = mix(centralColor.rgb, smoothColor, opacity);

        gl_FragColor = vec4(pow(smoothColor, vec3(0.96)), centralColor.a);
    }
}
